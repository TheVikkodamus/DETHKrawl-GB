...
Roll `3d6` for each stat. Don’t record the total — just use the table to determine a bonus.
...
### `d6` – Gear (Roll 3 times; duplicates = more of that item)
...
### `d12` – Weapons Table (Roll twice unless one weapon is bulky)
...
> Black powder weapons are deadly. They **ignore all armor**.  
> Takes **1 round to reload**, and **cannot reload** while in melee range.
...
- **Block `d6` damage** from an incoming hit  

> - You may spend **multiple dice** on a single roll.  
> - Cannot be used on a **natural 1** — failure is final.  
> - You may **gift one die** to another player at any time.
...
**Group**: Player and GM both roll `d6`. Higher side goes first.  
...
- Double damage taken & armor drops one tier (on defense)
...
| Half Cover       | 10            | 12              |
| 3/4 Cover        | 8             | 10              |
| Full Cover       | Can't be targeted (unless flanked or hit by area effects) |
...
- **Success** = BROKEN (roll on Broken Table)
- **Failure** = DEAD
